...
4. 普通战斗和精英战斗会进入卡牌战斗界面；营地节点进入独立营地页，可选择休息恢复最大生命的 30% 或升级一张未升级卡牌；特殊事件进入后会随机触发临时篝火、伏击战斗或流动商人；商店节点进入独立商店页，可花费金币购买卡牌或删除牌组中的一张牌。
...
- `engine/GameEngine.java`：战斗流程门面，负责按所选角色开始远征、选择地图节点、开始战斗、玩家出牌、敌方意图预告、敌方固定行动表执行、结束回合、抽牌洗牌、胜负判断、按当前角色奖励池和普通/罕见/稀有/传说权重生成奖励、跳过奖励、金币结算、营地休息/升级、特殊事件随机分支、商店购牌/删牌和离店结算。
...
- 商店离开后完成节点并解锁后续路线
- 特殊事件可随机触发临时篝火、伏击战斗或流动商人，并按对应流程完成节点
- 击败 Boss 后进入通关状态
